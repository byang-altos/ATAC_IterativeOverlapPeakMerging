...
if (is.null(opt$metadata) || is.null(opt$macs2dir) || is.null(opt$outdir) || is.null(opt$suffix) || is.null(opt$blacklist) || is.null(opt$genome) || is.null(opt$spm) || is.null(opt$rule)) {
  ...
  --chr_remove <chromosome to remove from peak set> (default \"Y\") \n
  --mito <mitochondrial chromosome name> (default \"MT\") \n
...
removeSpecificChromosome <- function(gr, remove = chr_remove){
...
df <- data.frame(
  ...
  groups = metadata_df$Group)
df$summits = file.path(macs2_dir, rownames(metadata_df), paste0(rownames(metadata_df), end))
...
for(i in seq_along(peakCalls[[1]])){
  ...
  #remove peaks that map to chr_remove
  gr_i <- removeSpecificChromosome(gr_i, remove = chr_remove)
  chrY <- orig_length - cliffed - length(gr_i)
  message(paste0(chr_remove, " - ", chrY))
...
#remove peaks that map to chr_remove
gr_merge <- removeSpecificChromosome(gr_merge, remove = chr_remove)
